...
<html>
	<head>
		<script>
			...
			{
			   var conf = confirm('Are you sure want to disapprove?');
			   if(conf)
			      window.location=anchor.attr("href");
			}
		</script>
		<meta charset="utf-8" />
		<title>Admin Page-Teacher-Approvement</title>
	...
	<body background="assets/images/coverpage4.jpg">
				...
				<div class="navbar-collapse nav-main-collapse collapse pull-right">
					<nav class="nav-main mega-menu">
						<ul class="nav nav-pills nav-main scroll-menu" id="topMain">
							<li class="active"><a href="page-admin-teacher-approvement-form.php">Teacher Approvement</a></li>
		...
		<div id="wrapper">
			...
			<section id="" class="padding60">
				<h2 align="center"><b>Teacher's Approvement List</b></h2>
				<div class="container">
					...
					<?php
                       ...
                       $sql = "SELECT * from temp_teacher WHERE '$id'=tmp_tcr_id";
                       $result = mysqli_query($db, $sql);
                       while($row = mysqli_fetch_assoc($result)){
                       Print '
                
						<form name="approvement-form" action="approvement.php" method="post">
							
							
								<input type="text" name="temp_id" value="'.$row['tmp_tcr_id'].'" readonly="readonly" hidden>
								<input type="text" name="tcrpass" value="'.$row['tmp_tcr_password'].'" readonly="readonly" hidden>
								<tr>
									<td>Teacher Name</td>
									<td><input type="text" name="tcrname" value="'.$row['tmp_tcr_name'].'" readonly="readonly" class="form-control"></td>
								</tr>
								<tr>
									<td>Teacher IC</td>
									<td><input type="text" name="tcric" value="'.$row['tmp_tcr_ic'].'" readonly="readonly" class="form-control"></td>
								</tr>
								<tr>
									<td>Gender</td>
									<td><input type="text" name="tcrgender" value="'.$row['tmp_tcr_sex'].'" readonly="readonly" class="form-control"></td>
								</tr>
								<tr>
									<td>Marital Status</td>
									<td><input type="text" name="tcrstatus" value="'.$row['tmp_tcr_status'].'" readonly="readonly" class="form-control"></td>
								</tr>
								<tr>
									<td>Email</td>
									<td><input type="text" name="tcremail" value="'.$row['tmp_tcr_email'].'" readonly="readonly " class="form-control"></td>
								</tr>
								<tr>
									<td>Address</td>
									<td><input type="text" name="tcraddress" value="'.$row['tmp_tcr_address'].'" readonly="readonly" class="form-control"></td>
								</tr>
								<tr>
									<td>Phone Number</td>
									<td><input type="text" name="tcrphone" value="'.$row['tmp_tcr_telnum'].'" readonly="readonly" class="form-control"></td>
								</tr>
							
					
					
				';}
				$sql1 = "SELECT subject_code FROM temp_teach WHERE tmp_tcr_id='$id' ";
				...
				?>
					...
					<div align="center">
							...
							<?php print'<a onclick="javascript:confirmationDisapprove($(this));return false;" href="disapprovement.php?id='.$id.'"><input type="button" name="btndisapprove" value="Disapprove" class="btn btn-primary" ></a>';?>
						</form>
					</div>
				</div>
				<br>
